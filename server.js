...
function findHTFSupportZones() {
  ...
  ['H1','H4','D1'].forEach(tf=>{
    ...
    for(let i=LR;i<data.length-LR;i++){
      ...
      if(isLow){
        // Check if price has bounced from this level multiple times
        const bouncePrice = data[i].low;
        let bounceCount = 0;
        for(let k=i+1;k<data.length;k++){
          if(Math.abs(data[k].low - bouncePrice) < data[i].low * 0.002) bounceCount++;
        }
        if(bounceCount >= 1){
  ...
  const clustered = [];
  htfData.sort((a,b)=>a.price-b.price);
  htfData.forEach(z=>{
    const existing = clustered.find(c=>Math.abs(c.price-z.price)/z.price<0.005);
    if(existing){
      existing.bounceCount += z.bounceCount;
      existing.tfs = [...new Set([...existing.tfs, z.tf])];
    } else {
      clustered.push({price:z.price, tfs:[z.tf], bounceCount:z.bounceCount,
        range:{lo:z.price*0.998, hi:z.price*1.002}});
    }
  });

  htfSupportZones = clustered.filter(z=>z.bounceCount>=1);
