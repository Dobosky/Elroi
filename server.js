...
// ── HTF SUPPORT ZONE DETECTION ────────────────────
// Only finds the most significant recent zones — max 5 total
function findHTFSupportZones() {
  const htfData = [];

  // Priority: D1 > H4 > H1
  const tfPriority = ['D1','H4','H1'];

  tfPriority.forEach(tf=>{
    const data = candles[tf];
    if(!data||data.length<10) return;
    const LR=3;
    // Only look at last 50 candles to keep it recent
    const start = Math.max(LR, data.length - 50);
    for(let i=start;i<data.length-LR;i++){
      let isLow=true;
      for(let j=i-LR;j<=i+LR;j++){
        if(j!==i&&data[j].low<=data[i].low){isLow=false;break;}
      }
      if(isLow){
        const bouncePrice = data[i].low;
        let bounceCount = 0;
        for(let k=i+1;k<data.length;k++){
          if(Math.abs(data[k].low - bouncePrice) / bouncePrice < 0.003) bounceCount++;
        }
        // Only include if price actually bounced from this level
        if(bounceCount >= 2){
          htfData.push({price:bouncePrice, tf, bounceCount, idx:i});
        }
      }
    }
  });

  // Cluster nearby zones (within 0.5% of each other = same zone)
  const clustered = [];
  htfData.sort((a,b)=>b.bounceCount-a.bounceCount); // strongest first
  htfData.forEach(z=>{
    const existing = clustered.find(c=>Math.abs(c.price-z.price)/z.price<0.005);
    if(existing){
      if(z.bounceCount > existing.bounceCount) existing.bounceCount = z.bounceCount;
      existing.tfs = [...new Set([...existing.tfs, z.tf])];
    } else {
      clustered.push({
        price: z.price,
        tfs: [z.tf],
        bounceCount: z.bounceCount,
        range: {lo: z.price * 0.9985, hi: z.price * 1.0015}
      });
    }
  });

  // Sort by proximity to current price, keep only 5 closest
  const sorted = clustered
    .filter(z => z.price < currentPrice * 1.05) // only below or just above price
    .sort((a,b) => Math.abs(a.price - currentPrice) - Math.abs(b.price - currentPrice))
    .slice(0, 5); // MAX 5 zones

  htfSupportZones = sorted;
  broadcastDash({type:'htf_zones', zones:htfSupportZones});
}

// ── HTF ZONE — UPTREND STRUCTURE DETECTION ────────
// When price is in/near HTF zone, detect if an upward structure forms
// Upward structure: price goes A→B, pulls back to C (C>A), then breaks above B again
function checkHTFZoneUptrend() {
  if(!htfSupportZones.length||!currentPrice) return;
  const data = candles['M1'];
  if(!data||data.length<20) return;

  // Find if price is near any HTF zone
  const nearZone = htfSupportZones.find(z=>
    currentPrice >= z.range.lo * 0.995 && currentPrice <= z.range.hi * 1.01
  );

  if(!nearZone) {
    // Price left the zone downward = resume if was paused
    if(htfZonePaused && currentPrice < htfSupportZones.reduce((min,z)=>Math.min(min,z.range.lo),Infinity)*0.998){
  ...
  if(swingLows.length>=2 && swingHighs.length>=1){
    ...
    if(lastLow.price > prevLow.price && lastLow.idx > prevLow.idx){
      ...
      if(currentPrice > lastHigh.price && lastHigh.idx > prevLow.idx){
        if(!htfZonePaused){
          log(`⚠ Uptrend structure in HTF zone at ${nearZone.price.toFixed(2)} — pausing bot`);
          htfZonePaused = true;
          setStatus('scanning','PAUSED — HTF ZONE');
          setTicker(`⚠ Uptrend forming in HTF support zone ${nearZone.price.toFixed(2)} — paused`);
          telegram(`⚠ <b>Bot paused</b>\nUptrend structure detected in HTF support zone\nZone: ${nearZone.price.toFixed(2)}\nWill resume on breakout or full uptrend confirmation`);
          broadcastState();
        }
      }
    }
...
function connectDeriv() {
  ...
  derivWs=new WebSocket(`wss://ws.binaryws.com/websockets/v3?app_id=${APP_ID}`);
  ...
  derivWs.on('message',(raw)=>{
    ...
    if(d.msg_type==='candles'){
      ...
      if(['H1','H4','D1'].includes(tf)) findHTFSupportZones();
